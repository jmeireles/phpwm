...
PHP_FUNCTION(phpwm_window_get_children) {
	int windownum;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, (char*)"l", &windownum) == FAILURE) {
		return;
	}
	std::vector<int> list = phpwm_window_get_children(windownum);
	std::vector<int>::iterator iter;
...
PHP_FUNCTION(phpwm_graphics_poly_rectangle){
	int windownum, x1, y1, x2, y2;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, (char*)"lllll", &windownum, &x1, &y1, &x2, &y2) == FAILURE) {
		return;
	}
	phpwm_graphics_poly_rectangle(windownum, x1, y1, x2, y2);
}
...
static function_entry app_functions[] = {PHP_FE(phpwm_window_subscribe_events, NULL) PHP_FE(phpwm_move_window, NULL)
	...
	PHP_FE(phpwm_graphics_poly_rectangle, NULL)
	PHP_FE(phpwm_window_get_children, NULL)
...
zend_module_entry app_module_entry = { STANDARD_MODULE_HEADER, (char*) "app", app_functions, NULL, /* MINIT */
NULL, /* MSHUTDOWN */
NULL, /* RINIT */
NULL, /* RSHUTDOWN */
NULL, /* MINFO */
(char*) "1.0", STANDARD_MODULE_PROPERTIES };
